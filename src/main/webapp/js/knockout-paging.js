...
(function (root, factory) {
  ...
  ko.extenders.paged = function(target, options) {
    ...
    // We use this field as a backing field for the pageNumber computed observable,
    // as we want to do some additional validation
    var _pageSize = ko.observable(options && options['pageSize'] || ko.paging.defaults.pageSize || 10);    
    ...
    target.pageGenerator = ko.paging.generators[options && options['pageGenerator'] || 'sliding'];
    ...
    target.pagingInfo = function() {
        var first = (target.pageNumber() - 1) * target.pageSize() + 1;
        var last = first + target.pageSize() - 1;
        if (last > target.itemCount())
            last = target.itemCount();
        return "共" + target.itemCount() + "项 , 当前为" + first + " - " + last + "项";
    };
